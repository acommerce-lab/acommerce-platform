...
public class AuthController : ControllerBase
{
        ...
        OpEngine engine)
    {
        ...
        _users = repoFactory.CreateRepository<VendorUser>();
        _engine = engine;
    }
    ...
    public async Task<IActionResult> VerifySms([FromBody] VerifySmsOtp req, CancellationToken ct)
    {
        ...
        var principal = new VendorPrincipal { UserId = user.Id.ToString(), DisplayName = user.FullName };
        var token = await _tokenStore.IssueAsync(principal, ct);

        var verifyOp = Entry.Create("auth.user_verify")
            .Describe($"Vendor {user.Id} verified via SMS OTP")
            .From("System:auth", 1, ("role", "auth_service"))
            .To($"VendorUser:{user.Id}", 1, ("role", "verified_user"))
