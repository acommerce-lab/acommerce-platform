...
```
Apps/Order.Api2/        Backend (ASP.NET Core 9 + SQLite + OpEngine + JWT + SMS 2FA)
...
Apps/Order.Web2/        Blazor Web App (Server interactive)
  Components/
  ...
  wwwroot/
    ...
    _framework/blazor.web.js  Self-hosted (the .NET 9 SDK doesn't ship it on
                              this image; same trick as Ashare.Web2)
...
class swap inside `Order.Api2/Program.cs` — the rest of the stack stays.
...
```bash
docker compose -f Apps/Order.Web2/docker-compose.yml up --build
```

Both services come up; the API persists its SQLite file in
`Apps/Order.Web2/_data/order.db`. Open http://localhost:5701.

## Smallest possible hosting (no Docker, no compose)

This is what we recommend pushing for the client preview today:

1. `dotnet publish Apps/Order.Api2 -c Release -o /opt/order/api`
2. `dotnet publish Apps/Order.Web2 -c Release -o /opt/order/web`
...
ExecStart=/usr/bin/dotnet Order.Api2.dll
...
ExecStart=/usr/bin/dotnet Order.Web2.dll
