...
/// <summary>
/// SMS OTP auth for provider login. Mirror of <c>Ejar.Api.Controllers.AuthController</c>;
/// the only differences are the JWT config (separate issuer/audience) and the
/// <c>role=provider</c> claim that backend authorization uses.
/// </summary>
[ApiController]
[Route("auth")]
public class AuthController : ControllerBase
{
    ...
    private readonly EjarProviderJwtConfig _jwt;
    private readonly ITwoFactorChannel _otpChannel;

    public AuthController(OpEngine engine, EjarProviderJwtConfig jwt, ITwoFactorChannel otpChannel)
    {
    ...
    public async Task<IActionResult> VerifyOtp([FromBody] OtpVerifyBody body, CancellationToken ct)
    {
        ...
        var op = Entry.Create("auth.otp.verify")
            ...
            .To($"Provider:{userId}", 1, ("role", "authenticated"))
            .Tag("user_id", userId).Tag("role", "provider")
            .Analyze(new RequiredFieldAnalyzer("phone", () => phone))
        ...
        var data = new { token, userId, name = user?.FullName ?? "", phone = MaskPhone(phone), role = "provider" };
        var env = await _engine.ExecuteEnvelopeAsync(op, data, ct);
        if (env.Operation.Status != "Success")
