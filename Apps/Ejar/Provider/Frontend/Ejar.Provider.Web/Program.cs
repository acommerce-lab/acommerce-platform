...
using Ejar.Provider.Web.Components;
using Ejar.Provider.Web.Interceptors;
using Ejar.Provider.Web.Interpreters;
using Ejar.Provider.Web.Operations;
using Ejar.Provider.Web.Services;
using Ejar.Provider.Web.Store;
...
builder.Services.AddScoped<EjarProviderCircuitHttp>();

// ─── OpEngine للعمليات المحلّية ────────────────────────────────────────
builder.Services.AddScoped<OpEngine>(sp =>
    new OpEngine(sp, sp.GetRequiredService<ILogger<OpEngine>>()));

// ─── HTTP → Ejar Provider API (port 5310 by default) ──────────────────
var apiBase = builder.Configuration["EjarProviderApi:BaseUrl"] ?? "http://localhost:5310";
builder.Services.AddHttpClient("ejar-provider", c =>
{
...
{
    var circuit = sp.GetRequiredService<EjarProviderCircuitHttp>();
...
builder.Services.AddScoped<EjarProviderRealtimeService>();
...
{
    opts.HttpClientName    = "ejar-provider";
