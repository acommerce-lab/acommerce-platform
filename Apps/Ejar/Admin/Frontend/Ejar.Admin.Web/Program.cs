...
using Ejar.Admin.Web.Components;
using Ejar.Admin.Web.Interceptors;
using Ejar.Admin.Web.Interpreters;
using Ejar.Admin.Web.Operations;
using Ejar.Admin.Web.Services;
...
builder.Services.AddScoped<EjarAdminCircuitHttp>();

// ─── OpEngine للعمليات المحلّية ────────────────────────────────────────
builder.Services.AddScoped<OpEngine>(sp =>
    new OpEngine(sp, sp.GetRequiredService<ILogger<OpEngine>>()));

// ─── HTTP → Ejar Provider API (port 5320 by default) ──────────────────
var apiBase = builder.Configuration["EjarAdminApi:BaseUrl"] ?? "http://localhost:5320";
builder.Services.AddHttpClient("ejar-admin", c =>
{
...
{
    var circuit = sp.GetRequiredService<EjarAdminCircuitHttp>();
...
builder.Services.AddScoped<EjarAdminRealtimeService>();
...
{
    opts.HttpClientName    = "ejar-admin";
