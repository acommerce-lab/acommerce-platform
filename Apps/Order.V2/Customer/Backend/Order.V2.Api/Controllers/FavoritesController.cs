...
public class FavoritesController : ControllerBase
{
    ...
    public async Task<IActionResult> Toggle([FromBody] ToggleRequest req, CancellationToken ct)
    {
        ...
        if (existing.Count > 0)
        {
            ...
            var removeOp = Entry.Create("favorite.remove")
                ...
                .Execute(async ctx => { await _repo.DeleteAsync(fav, ctx.CancellationToken); })
                .Build();
        ...
        var newFav = new Favorite
        {
            ...
            UserId = req.UserId, OfferId = req.OfferId
        };
    ...
    public async Task<IActionResult> ByUser(Guid userId, CancellationToken ct)
    {
        ...
        {
            o.Id, o.Title, o.Description, o.Price, o.OriginalPrice, o.Currency, o.Emoji,
            DiscountPercent = o.DiscountPercent,
            Vendor = vendors.TryGetValue(o.VendorId, out var v)
                ? new { v.Id, v.Name, v.LogoEmoji, v.City, v.District }
                : null
        }).ToList();
