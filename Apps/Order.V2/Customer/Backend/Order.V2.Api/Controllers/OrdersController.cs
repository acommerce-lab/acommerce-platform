...
public class OrdersController : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] CreateOrderRequest req, CancellationToken ct)
    {
        ...
        {
            Id = orderId, CreatedAt = DateTime.UtcNow,
            OrderNumber = orderNumber,
            CustomerId = req.CustomerId, VendorId = vendor.Id,
            ...
            Subtotal = subtotal, Total = subtotal, Currency = "SAR",
            CustomerNotes = req.CustomerNotes, Status = OrderStatus.Pending,
        };
            ...
            .Execute(async ctx =>
            {
                ...
                foreach (var (offer, qty) in offers)
                {
                    await _items.AddAsync(new OrderItem
                    {
                        ...
                        OrderId = record.Id, OfferId = offer.Id,
                        OfferTitle = offer.Title, Emoji = offer.Emoji,
                        Quantity = qty, UnitPrice = offer.Price, LineTotal = offer.Price * qty
        ...
        {
            record.Id, record.OrderNumber, record.Subtotal, record.Total,
            record.Currency, record.PickupType, record.PreferredPayment,
            record.CashTendered, ExpectedChange = record.ExpectedChange,
            VendorName = vendor.Name, Status = record.Status.ToString()
        });
    }
    ...
    private async Task<IActionResult> MapOrderList(IReadOnlyList<OrderRecord> orders, CancellationToken ct)
    {
        ...
        {
            o.Id, o.OrderNumber, o.Total, o.Currency, o.Status,
            StatusAr = StatusToArabic(o.Status),
            o.PickupType, o.PreferredPayment, o.CashTendered,
            ExpectedChange = o.ExpectedChange, o.CreatedAt,
    ...
    public async Task<IActionResult> Get(Guid id, CancellationToken ct)
    {
        ...
        {
            o.Id, o.OrderNumber, o.Total, o.Subtotal, o.Currency, o.Status,
            StatusAr = StatusToArabic(o.Status),
            o.PickupType, o.PreferredPayment, o.CashTendered,
            ExpectedChange = o.ExpectedChange,
            o.CarModel, o.CarColor, o.CarPlate, o.CustomerNotes, o.CreatedAt,
    ...
    public async Task<IActionResult> Accept(Guid id, CancellationToken ct)
    {
        ...
        var op = Entry.Create("order.accept")
            ...
            .Execute(async ctx =>
            {
                o.Status = OrderStatus.Accepted; o.UpdatedAt = DateTime.UtcNow;
    ...
    public async Task<IActionResult> Ready(Guid id, CancellationToken ct)
    {
        ...
        var op = Entry.Create("order.ready")
            ...
            .Execute(async ctx =>
            {
                o.Status = OrderStatus.Ready; o.UpdatedAt = DateTime.UtcNow;
                await _orders.UpdateAsync(o, ctx.CancellationToken);
                ctx.Set("orderId", o.Id);
            })
            .Build();
    ...
    public async Task<IActionResult> Deliver(Guid id, CancellationToken ct)
    {
        ...
        var op = Entry.Create("order.deliver")
            ...
            .Execute(async ctx =>
            {
                o.Status = OrderStatus.Delivered; o.UpdatedAt = DateTime.UtcNow;
                await _orders.UpdateAsync(o, ctx.CancellationToken);
                ctx.Set("orderId", o.Id);
            })
            .Build();
    ...
    public async Task<IActionResult> Cancel(Guid id, CancellationToken ct)
    {
        ...
        var op = Entry.Create("order.cancel")
            .Describe($"Order {o.OrderNumber} cancelled")
    ...
    public async Task<IActionResult> VendorCallback(Guid id, [FromBody] VendorCallbackRequest req, CancellationToken ct)
    {
        ...
        var op = Entry.Create(opType)
            ...
            .Execute(async ctx =>
            {
                o.Status = newStatus.Value; o.UpdatedAt = DateTime.UtcNow;
                await _orders.UpdateAsync(o, ctx.CancellationToken);
            })
            .Build();
