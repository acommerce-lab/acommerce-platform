...
/// <summary>
/// IRealtimeTransport يربط <see cref="AshareV3RealtimeHub"/> الفعليّ بمنطق الكيت
...
/// تطبيقنا يخدم <c>AshareV3RealtimeHub : AShareHub</c> على <c>/realtime</c>،
...
/// (لأنّ كلّ الاتصالات مفتوحة على AshareV3RealtimeHub). كلّ <c>SendToGroupAsync</c>
/// يبثّ إلى group فارغة، فلا يصل أحد. هذه القشرة ترفع IHubContext إلى
/// <see cref="AshareV3RealtimeHub"/> الفعليّ فيُحلّ المشكل.</para>
/// </summary>
public sealed class AshareV3SignalRTransport : IRealtimeTransport
{
    private readonly IHubContext<AshareV3RealtimeHub> _hub;

    public AshareV3SignalRTransport(IHubContext<AshareV3RealtimeHub> hub) => _hub = hub;
